...
# WRITE YOUR SOLUTION BELOW:
CHILD_INPUTS='[{"txid":"'$PARENT_TXID'","vout":1}]'
...
CHILD_FEE_SATS=$((CHILD_TX_SIZE * 20))
...
SECONDARY_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$SECONDARY_TX" | jq -r '.vout[0].value * 100000000' | cut -d. -f1)
...
TIMELOCK_AMOUNT=$((SECONDARY_OUTPUT_VALUE - 1000))
